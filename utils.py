...
def load_data(dataset, data_dir='./data', transform=None, simulate_images=True, num_workers=None):
    ...
    参数:
        dataset (str): 数据集名称，例如 'copydays', 'scid'
        data_dir (str): 数据目录
    ...
    elif dataset.lower() == 'scid':
        result = load_scid(data_dir, transform, simulate_images, num_workers)
    elif dataset.lower() == 'copy2':
        result = load_copy2(data_dir, transform, simulate_images, num_workers)
    elif dataset.lower() == 'screencopynet':
        result = load_screencopynet(data_dir, transform, simulate_images, num_workers)
    else:
...
def load_scid(data_dir='./data', transform=None, simulate_images=True, num_workers=None):
    ...
    # 计算总耗时
    total_time = time.time() - start_time
    print(f"SCID 数据集加载完成，总耗时: {total_time:.2f}秒")
    print(f"- 查询图像: {len(query_images)}张")
    print(f"- 数据库图像: {len(db_images)}张")
    print(f"- 正样本对: {len(positives)}对")
...
def load_copy2(data_dir='./data', transform=None, simulate_images=True, num_workers=None):
    ...
    # 计算总耗时
    total_time = time.time() - start_time
    print(f"Copy2 数据集加载完成，总耗时: {total_time:.2f}秒")
    print(f"- 查询图像: {len(query_images)}张")
    print(f"- 数据库图像: {len(db_images)}张")
    print(f"- 正样本对: {len(positives)}对")
...
def load_screencopynet(data_dir='./data', transform=None, simulate_images=True, num_workers=None):
    """
    加载 Copy2 数据集
    ...
    返回:
        dict: 包含 ScreenCopyNet 数据集信息的字典
    """
    # 开始计时
    start_time = time.time()
    ...
    # 加载 ground truth 数据
    gnd_start_time = time.time()
    gnd_path = os.path.join(data_dir, 'screencopynet', 'gnd_screencopynet.pkl')
    ...
    # 图像目录 - 修改为正确的图像目录
    img_dir = os.path.join(data_dir, 'ScreenCopyNet', 'jpg')
    ...
    # 计算总耗时
    total_time = time.time() - start_time
    print(f"ScreenCopyNet 数据集加载完成，总耗时: {total_time:.2f}秒")
    print(f"- 查询图像: {len(query_images)}张")
    print(f"- 数据库图像: {len(db_images)}张")
    print(f"- 正样本对: {len(positives)}对")
