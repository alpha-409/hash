...
class ImageAttacker:
    ...
    def apply_strong_attacks(self, image_path, num_attacks=5):
        ...
        for attack_func in selected_attacks:
            ...
            if attacked_img.mode == 'RGBA':
                background = Image.new('RGB', attacked_img.size, (255, 255, 255))
                background.paste(attacked_img, mask=attacked_img.split()[3])
                attacked_img = background
    ...
    def _apply_noise(self, img):
        ...
        for _ in range(num_pixels):
            ...
            for i in range(min(c, 3)):  # Ensure we only modify RGB channels, not alpha
