...
def load_scid(data_dir='./data', transform=None, simulate_images=True, num_workers=None):
    ...
    参数:
        ...
        num_workers (int, optional): 用于并行加载图像的工作线程数
    ...
    # 图像目录
    img_dir = os.path.join(data_dir, 'SCID')
    ...
    def load_image(img_info):
        ...
        if os.path.exists(full_path):
            try:
                ...
                if transform:
                    img = transform(img)
                return idx, img, full_path, False  # 成功加载
            except Exception as e:
                if not is_query:
                    print(f"加载图像 {full_path} 时出错: {e}")
                if simulate_images:
                    img_tensor = torch.randn(3, 224, 224)
                    return idx, img_tensor, full_path, True  # 使用模拟图像
                return idx, None, full_path, True  # 加载失败
        else:
            if simulate_images:
                img_tensor = torch.randn(3, 224, 224)
                return idx, img_tensor, full_path, True  # 使用模拟图像
            return idx, None, full_path, True  # 图像不存在
    ...
    for q_idx, variants in enumerate(gnd):
        ...
        for variant_type in ['strong', 'crops', 'jpegqual']:
            for db_name in variants[variant_type]:
                # 找到数据库图像的索引
                try:
                    db_idx = imlist.index(f"attacked_images/{variant_type}_attack/{db_name}")
                    positives.append((q_idx, db_idx))
                except ValueError:
                    print(f"警告: 在数据库图像列表中找不到图像 {db_name}")
